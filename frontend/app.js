...
const modules = {
  students: {
    ...
    formFields: [
      { name: "StudentID", autoGenerated: true },
  ...
  exams: {
    ...
    formFields: [
      { name: "ExamID", autoGenerated: true },
  ...
  departments: {
    ...
    formFields: [
      { name: "DeptID", autoGenerated: true },
  ...
  courses: {
    ...
    formFields: [
      { name: "CourseID", autoGenerated: true },
  ...
  proctors: {
    ...
    formFields: [
      { name: "ProctorID", autoGenerated: true },
  ...
  attempts: {
    ...
    formFields: [
      { name: "AttemptNo", autoGenerated: true },
  ...
  results: {
    ...
    formFields: [
      { name: "ResultID", autoGenerated: true },
  ...
  violations: {
    ...
    formFields: [
      { name: "ViolationID", autoGenerated: true },
  ...
  instructors: {
    ...
    formFields: [
      { name: "InstructorID", autoGenerated: true },
      { name: "FirstName", required: true },
      { name: "LastName", required: true },
  ...
  questionbanks: {
    ...
    formFields: [
      { name: "BankID", autoGenerated: true },
  ...
  questions: {
    ...
    formFields: [
      { name: "QuestionID", autoGenerated: true },
  ...
  examsessions: {
    ...
    formFields: [
      { name: "SessionID", autoGenerated: true },
      { name: "AttemptNo", type: "number", required: true },
      { name: "StudentID", required: true },
      { name: "ExamID", required: true },
  ...
  notifications: {
    ...
    formFields: [
      { name: "NotifID", autoGenerated: true },
      { name: "StudentID", required: true },
  ...
  violationcategories: {
    ...
    formFields: [
      { name: "CategoryID", autoGenerated: true },
...
function validateFormFields(config, payload, isEdit) {
  for (const field of config.formFields) {
    if (field.autoGenerated) continue;
    const val = payload[field.name];
    const isEmpty = val === null || val === undefined || val === "";

    if (field.required && !isEdit && isEmpty) {
  ...
  entityForm.innerHTML = `
    ...
    ${isEdit ? `
      <div class="form-group full">
        <label>ProctorID <span style="font-size:0.75rem;opacity:0.6">(auto-generated)</span></label>
        <input type="text" value="${normalizeValue(getFieldValue(record, 'ProctorID'))}" disabled style="opacity:0.6;cursor:not-allowed" />
      </div>
    ` : ""}
    ${fieldsToRender
      .filter((fieldName) => fieldName !== "ProctorID")
      .map((fieldName) => {
        ...
        return `
          <div class="form-group ${field.full ? "full" : ""}">
            <label>${field.name}${field.required ? " <span style='color:#e05;font-size:0.8rem'>*</span>" : ""}</label>
            ${fieldInput(field, currentValue)}
          </div>
        `;
      })
      .join("")}
...
function openForm(config, record = null) {
  ...
  entityForm.innerHTML = `
    ${config.formFields
      .map((field) => {
        // Auto-generated PK: skip entirely on Add; show as read-only badge on Edit
        if (field.autoGenerated) {
          if (!isEdit) return ""; // hidden on add
          const currentValue = getFieldValue(record, field.name);
          return `
            <div class="form-group full">
              <label>${field.name} <span style="font-size:0.75rem;opacity:0.6">(auto-generated)</span></label>
              <input type="text" value="${normalizeValue(currentValue)}" disabled style="opacity:0.6;cursor:not-allowed" />
            </div>
          `;
        }
  ...
  entityForm.onsubmit = async (event) => {
    ...
    const validationError = validateFormFields(config, payload, isEdit);
    if (validationError) {
